...
class general_setup:
    ...
    def __init__(self, src_dir,QSM_dir, QSM_base, fin_dir, t_name, l_type, current_shoots):
        ...
        if self.cs_check:
            ...
            self.pos_path = self.pos_path + '.csv'
        self.wooden_obj_orig_path = os.path.join(QSM_dir, t_name + QSM_base)  # path to obj file with wooden skeleton generated by P. Sloup algorithm
        ...
        if l_type != '':
            leaf_type_d = '_%s' % l_type
        else:
            leaf_type_d = ''
        self.tree_obj_path = os.path.join(self.final_dir,t_name+'_whole_tree%s.obj' % leaf_type_d) # path to the obj file with components of whole tree (current and older shoots and  wooden skeleton)
    ...
    def get_files(self, tree_name, veg_path):
        ...
        for i_file in t_list:
            t_split = i_file.split('_')
            if len(t_split) > 1:
                t_name = i_file.split('_')[0]
                t_cat = i_file.split('_')[-1]
                t_form = i_file.split('.')
                if t_name == tree_name and len(t_form) == 1:
                    if t_cat[0] == 'l':  # j means that is point cloud with needles (jehlice)
                        file_list_veg.append(i_file)

                    elif t_cat[0] == 'w':  # k means the point cloud of wooden parts (kmen (asi))
                    ...
                    b_i_path = os.path.join(self.source_dir, 'tmp', '%s_orig.obj' % i_file)
...
class Tree:
    ...
    def __init__(self, t_name, cluster_cube_size, lai, hull_cube_size, general_tree_setup, leaf_cls):
    ...
    def initiate_tree(self, max_vegetation):
        ...
        self.shoot_number = (2*self.LAI*self.crown_projection_area)/self.leaf.l_area
    ...
    def load_points(self, veg_path):
        ...
        start_time = time.time()
        t_file_name = os.path.join(veg_path, 'tmp', self.name+'_all.obj')
    ...
    def find_projection_area(self):
        ...
        print('center: ',  [float(x) for x in self.center]) # just for printing purposes
        print('It takes %s seconds.' % (time.time()-start_time))
        print('--------------------------------------------------------------')
    ...
    def write_shoots(self, dist_file, description):
        ...
        # shoot_file = open(self.leaf.shoot_path, 'r')
        ...
        for i_row in tqdm(dist_file):
            if self.leaf.l_form == 'three_leaves':
                leaf_name_arr_tmp = [self.leaf.l_arr[0],self.leaf.l_arr[0],self.leaf.l_arr[1],self.leaf.l_arr[2]] # because the average leaf should have 50% chance and the smaller and larger only 25%
                leaf_name = choice(leaf_name_arr_tmp)
            elif self.leaf.l_form == 'multiple_leaves':
                leaf_name = choice(self.leaf.l_arr)
            else:
                leaf_name = self.leaf.l_arr
            try:
                leaf_file = open(os.path.join(self.leaf.l_dir,leaf_name), 'r')
            except FileNotFoundError:
                print(Style.BRIGHT+Fore.MAGENTA+'Leaf obj file is not found. Check the filename and path.'+Style.RESET_ALL)
            leaf_csv_file = csv.reader(leaf_file, delimiter=' ')
            leaf_vox_arr = self.load_shoot_voxels(leaf_csv_file)
            leaf_file.close()
            leaf_file = open(os.path.join(self.leaf.l_dir,leaf_name), 'r')
            leaf_csv_file = csv.reader(leaf_file, delimiter=' ')
            leaf_vox_num = len(leaf_vox_arr)
            sc_factor = [uniform(1,2),uniform(0.82,1.17)]
            trans_info = [float(i_row[0]), float(i_row[1]), float(i_row[2])]
            rot_info = float(i_row[-1])
            trans_vox_arr = self.transform(leaf_vox_arr, trans_info, rot_info)
            ind_vox = 0
            for i_row_leaf in leaf_csv_file:
                if i_row_leaf[0] == 'v':
                    t_line = ['v']+trans_vox_arr[ind_vox]
                    ind_vox += 1
                elif i_row_leaf[0] == 'f':
                    vox_1 = str(int(float(i_row_leaf[1])+ind_leaf*leaf_vox_num+last_voxel_id))
                    vox_2 = str(int(float(i_row_leaf[2])+ind_leaf*leaf_vox_num+last_voxel_id))
                    vox_3 = str(int(float(i_row_leaf[3])+ind_leaf*leaf_vox_num+last_voxel_id))
...
class Leaf:
    """
    There are three approaches on how to distribute leaf objects:
        1) single_leaf - using only one shoot/leaf 3D object and distribute it within a tree crown
        2) three_leaves - using three sizes of leaves - small, average, larger. The leaf/shoot objects will be present with probability  of 25% for small leaf/shoot, 50% average leaf/shoot, and 25% larger leaf/shoot object
        3) multiple_leaves - list of shoot/leaf objects will be distributed with equal probability within the tree crown
        notes:
            - in all cases the average leaf/shoot area of all object used should be the same as is set in Main.py
            - in all cases there is implemented a small scaling of all the leaf/shoot objects - to introduce small variability
    """
    def __init__(self, leaf_dir,leaf_arr, leaf_area, leaf_form):
        self.l_dir = leaf_dir # path to the leaf/shoot object/s
        self.l_arr = leaf_arr # list of shoot/leaf object, which should be used in the 3D tree object
        self.l_area = leaf_area # shoot/leaf area computed by blender
        self.l_form = leaf_form # form of leaf objects - see details in the comment of the Leas class
        # self.name = t_name # name of the tree (ID or whatever)
